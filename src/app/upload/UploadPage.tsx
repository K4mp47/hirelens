...
export default function UploadPage() {
  ...
  const handleAnalyze = async ({ companyName, jobTitle, jobDescription, uploadFile }: ReviewFormValues) => {
    ...
    try {
      ...
      if (!uploadedFile) return fail("The PDF could not be uploaded. Try again.");
      ...
      if (!uploadedImage) return fail("The resume preview could not be stored.");
      ...
      if (!feedback) return fail("Puter.js did not return feedback. Retry the analysis.");
      ...
      if (!feedbackText) return fail("The AI response could not be read.");
      ...
      fail("The analysis could not be completed. Check your connection and try again.");
    }
  };

  const selectedFile = form.watch("uploadFile");
      ...
      <main className="mx-auto grid w-full max-w-7xl gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[0.75fr_1.25fr] lg:px-8 lg:py-12">
        <aside className="space-y-6 lg:sticky lg:top-24 lg:self-start">
          <div className="space-y-3">
            <p className="text-sm font-medium text-primary">New resume review</p>
            <h1 className="text-3xl font-semibold tracking-tight sm:text-4xl">Match your resume to a specific role.</h1>
            <p className="leading-7 text-muted-foreground">Provide the exact context an employer will use. HireLens preserves the existing Puter.js workflow and stores the review in your Puter account.</p>
          </div>
          <Card>
            <CardHeader><CardTitle className="text-base">What happens next</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              {analysisSteps.map((item, index) => <div key={item} className="flex gap-3"><span className="grid size-7 shrink-0 place-items-center rounded-full bg-primary/10 text-xs font-semibold text-primary">{index + 1}</span><p className="text-sm leading-6 text-muted-foreground">{item}</p></div>)}
            </CardContent>
          </Card>
        ...
        {isProcessing ? (
          <Card className="min-h-[520px]">
            <CardContent className="flex h-full min-h-[520px] flex-col items-center justify-center gap-6 p-8 text-center" aria-live="polite">
              <div className="grid size-16 place-items-center rounded-2xl bg-primary/10 text-primary"><LoaderCircle className="size-8 animate-spin motion-reduce:animate-none" /></div>
              <div className="max-w-md space-y-2"><h2 className="text-2xl font-semibold">Review in progress</h2><p className="text-muted-foreground">{statusText}</p></div>
              <div className="w-full max-w-md space-y-3"><Progress value={(step + 1) * 25} /><p className="text-xs text-muted-foreground">Step {step + 1} of {analysisSteps.length}. Progress reflects workflow stages, not Puter model completion.</p></div>
            </CardContent>
          </Card>
        ) : (
          <Card>
            <CardHeader><CardTitle>Role and resume details</CardTitle><CardDescription>All fields are required. Use the full job description for a more relevant comparison.</CardDescription></CardHeader>
            <CardContent>
              {statusText ? <div role="alert" className="mb-6 rounded-lg border border-destructive/40 bg-destructive/10 p-3 text-sm text-destructive">{statusText}</div> : null}
              <Form {...form}>
